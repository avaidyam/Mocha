...
@implementation BINInspectorBar {
	NSMutableIndexSet *_selectedIndexes;
	BINInspectorBarItem *_pressedItem;
...
@synthesize changedSelectionHandler = _changedSelectionHandler;
...
- (BINInspectorBarItem *)itemAtX:(CGFloat)x {
...
- (void)setItems:(NSArray *)newItems {
	if (newItems != _items) {
		...
		for (BINInspectorBarItem *item in _items) {
			item.tabBar = self;
		}
...
- (void)deselectItem:(BINInspectorBarItem *)item {
	if ([_items containsObject:item]) {
		NSUInteger index = [_items indexOfObject:item];
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	for (NSUInteger i = 0; i < [_items count]; i++) {
		BINInspectorBarItem *item = [_items objectAtIndex:i];
...
- (void)notify {
	...
	if (self.changedSelectionHandler) {
		self.changedSelectionHandler();
	}
}

- (void)mouseDown:(NSEvent *)theEvent {
	CGPoint point = [self convertPoint:[theEvent locationInWindow] fromView:nil];
	BINInspectorBarItem *item = [self itemAtX:point.x];
	if (item) {
		_dragging = YES;
		_pressedItem = item;
		if (_multipleSelection) {
...
@implementation BINInspectorBarItem
...
+ (BINInspectorBarItem *)itemWithIcon:(NSImage *)image tooltip:(NSString *)tooltipString {
	return [[BINInspectorBarItem alloc] initWithIcon:image tooltip:tooltipString];
}
